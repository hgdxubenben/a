function Ibc = BestCandiPartialLen(upR, nblocks, stm, width, Znum)
...
% % minlen = min(stm(I1).len, stm(I2).len);
% % if minlen == stm(I1).len
% %     minI2 = fdiffMU_1D(stm(I2).stream(1,1:minlen,:),upR(end,1:minlen,:))/minlen;
% % else
% %     minI1 = fdiffMU_1D(stm(I1).stream(1,1:minlen,:),upR(end,1:minlen,:))/minlen;
% % end
% % if minI2 < minI1
% %     Ibc(1) = I2;
% %     Ibc(2) = I1;
